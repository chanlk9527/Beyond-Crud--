...
图 1-10：箭头表示派生或展示关系。报名与提醒依赖已发布开课瞬间，录播期限依赖实际结束；图中日期均在 2027 年。
...
来源层分别改变设备时区与设备时钟。展示选择“跟随设备”时，从上海切到东京应只改变当地表示；用户固定选择纽约时区时，旅行不应覆盖该偏好。让客户端快十分钟，报名资格仍由服务端判断。再模拟服务端失去同步且时间质量超过业务允许范围，检查是否按约定暂停受理或转交判定，而不是继续假设服务端永远准确。

执行层检查保存失败不返回受理成功、旧提醒不会因改期而误发、已有播放与新播放按各自规则处理。对提醒执行器模拟钟表向前跳过计划点和向后回拨，分别检查迟到处理与重复执行防护。这些性质涉及事务、并发和调度，应由实际业务集成测试验证。**单独验证时间库不会证明它们已经成立**。
...
如果设计中某个字段的用途说不清，或某项测试找不到对应的业务约定，就值得回头检查。下面用设计题和故障诊断题练习这个方法。
