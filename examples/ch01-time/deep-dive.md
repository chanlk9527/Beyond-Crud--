...
往返还要覆盖编辑操作。数据库保留微秒，表单只展示到秒，原样提交也可能丢失精度。展示格式不应决定保存格式；如果还用时间戳检测并发修改，这种损失的影响留到乐观并发控制章节讨论。
...
[正文业务日查询](../../manuscript/part-01-values/01-time/05-business-day.md#sec-1-5-3)选择了支付时间，回答的是“这一天支付的订单金额”。如果运营要的是“这一天创建的订单金额”，就应换用创建事件。日期边界相同，选取的事实仍可能不同。
...
假设运营把课程从 7 月 10 日改到 7 月 11 日。新的开课瞬间、报名截止和提醒都顺延一天，`schedule_revision` 从 3 增至 4；已经成功受理的报名保留。旧提醒即使留在队列中，执行器也能通过版本不符识别它。版本检查如何与发送协调，留到乐观并发控制和消息章节。
